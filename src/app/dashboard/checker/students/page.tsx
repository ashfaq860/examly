// /dashboard/checker/students — register/manage the teacher's own class
// roster. This is what populates the "Add submission" roster dropdown
// (api/checker/papers' roster query, owner_id-scoped) — before this page
// existed there was no way to add a `students` row at all; every teacher
// had to type free-text name/roll on every single submission.
'use client';
...
export default function StudentsPage() {
  ...
  const fetchStudents = useCallback(async () => {
    const res = await fetch('/api/checker/students');
    if (res.ok) {
  ...
  const saveEdit = async (id: string) => {
    ...
    setSavingId(id);
    try {
      const res = await fetch(`/api/checker/students/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const toggleActive = async (s: Student) => {
    setSavingId(s.id);
    try {
      const res = await fetch(`/api/checker/students/${s.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="chk-root chk-students">
      <Link href="/dashboard/checker" className="chk-back"><ArrowLeft size={14} /> Paper Checker</Link>
