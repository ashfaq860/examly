...
export default function FullSubjectQuizPage() {
  ...
  const [resolvedSubjectUUID, setResolvedSubjectUUID] = useState<string|null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // instead of blindly taking the first slug match (which could pull
        // chapters belonging to an unrelated class's same-named subject).
        ...
        setResolvedSubjectUUID(subject.id);

        const urduSubjects = ["urdu","islamiyat","islamyat","pakistan studies"];
        const isUrdu = urduSubjects.includes(subject.name.toLowerCase());
        setForceUrdu(isUrdu); setLang(isUrdu ? "ur" : "en");

        const csIds = [cs.id];

        const { data: chapters } = await supabase.from("chapters").select("id").in("class_subject_id", csIds);
        const chapterIds = (chapters || []).map((c: any) => c.id);
        if (chapterIds.length === 0) { setError("No chapters found."); setLoading(false); return; }

        const { data: topics } = await supabase.from("topics").select("id").in("chapter_id", chapterIds);
        const topicIds = (topics || []).map((t: any) => t.id);
        if (topicIds.length === 0) { setError("No topics found."); setLoading(false); return; }

        const { data: qs } = await supabase.from("questions").select("*")
          .in("topic_id", topicIds).eq("question_type", "mcq");
        if (!qs || qs.length === 0) { setError("No questions found for this subject."); setLoading(false); return; }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setScore(finalScore);
    setSubmitted(true);
    if (user && resolvedSubjectUUID) {
      await supabase.from("results").insert([{
        user_id: user.id, subject_id: resolvedSubjectUUID,
        score: finalScore, total_questions: questions.length, in_progress: false,
      }]);
    }
    setSaving(false);
  }, [submitted, questions, answers, user, supabase, resolvedSubjectUUID]);
    ...
    <>
      ...
      <div className="d-flex align-items-center justify-content-center" style={{ minHeight:"100vh", background:"#f8fafc" }}>
        <div className="text-center">
          ...
          <p className="fw-semibold text-muted">Loading full subject quiz…</p>
        </div>
      </div>
      <Footer />
    </>
          ...
          <div style={{ background:"#fff", borderRadius:24, padding:"2.5rem", maxWidth:460, width:"100%", textAlign:"center", boxShadow:"0 8px 40px rgba(15,23,42,0.12)", border:"1px solid #e2e8f0" }}>
            {swimmerDead && !submitted ? (
              <>
                ...
                <p className="text-muted mb-4">Full Subject Quiz • 3 consecutive wrong answers</p>
              </>
            ) : (
              <>
                ...
                <p className="text-muted mb-4">Full Subject Quiz • {subjectName}</p>
              </>
            )}
