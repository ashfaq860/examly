...
export default function CubeSlider({ slides = defaultSlides, autoRotateInterval = 6000, transitionMs = 1200 }) {
  ...
  useEffect(() => {
    // Start auto-rotate only on mouse enter
    const node = containerRef.current;
    if (!node) return;

    node.addEventListener("mouseenter", startAutoRotate);
    node.addEventListener("mouseleave", stopAutoRotate);

    return () => {
      stopAutoRotate();
      node.removeEventListener("mouseenter", startAutoRotate);
      node.removeEventListener("mouseleave", stopAutoRotate);
    };
  }, []);
  ...
  return (
    <div className="slider-container position-relative overflow-hidden" ref={containerRef}>
      ...
      {/* Enhanced Responsive Styles - OPTIMIZED */}
      <style jsx>{`
        .slider-container { 
          height: 600px; 
          overflow: hidden; 
        ...
        /* Floating Image Style - No animation to improve performance */
        ...
        /* Perspective Image Style - Static transform */
        ...
        /* Image Glow Effect - Reduced opacity */
        .image-glow {
          position: absolute;
          top: 50%;
          left: 50%;
        ...
        /* Floating Elements - HIDDEN for performance */
        .floating-element {
          display: none;
        }
