...
export default function AcademyLayout({ children }: any) {
  ...
  const renderTrialStatus = () => {
    ...
    if (message) {
      return (
        <div className="text-center">
          <small className={`text-${darkMode ? "warning" : "primary"} fw-bold`}>{message}</small>
        </div>
      );
    }
    ...
    return (
      <div className="text-center">
        ...
        <div className="mb-1">
          <small className="text-muted d-block">Papers available:</small>
          <span className="fw-bold">{papersRemaining === "unlimited" ? "Unlimited" : papersRemaining}</span>
        </div>
        <div className="mb-1">
          <small className="text-muted d-block">Papers generated:</small>
        ...
        <div className="mt-2 pt-2 border-top border-secondary">
          ...
          <div className="fw-bold text-warning">No active plan</div>
        </div>
        <div className="mt-2">
          <Link href="/dashboard/packages" className="text-decoration-none">
    ...
    <div className={darkMode ? "bg-dark text-light" : "bg-light text-dark"}>
      ...
      >
        <div className="offcanvas-header">
          <h5 className="offcanvas-title fw-bold text-primary" id="offcanvasLabel">
        ...
        <div className="offcanvas-body">
          <ul className="nav nav-pills flex-column">
            {sidebarLinks.map((item) => (
              <li key={item.path} className="mb-2">
                <Link
                  href={item.path}
                  className={`nav-link sidebar-link d-flex align-items-center ${isActive(item.path) ? "active" : ""}`}
          ...
          <div className="mt-4 pt-3 border-top">
            <Suspense fallback={<div className="text-center text-muted">Loading plan...</div>}>
              <div className="trial-status-card">{renderTrialStatus()}</div>
            </Suspense>
          </div>
        </div>
